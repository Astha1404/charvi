...
/*this is delet quer*/
mysqli_query($con,"delete from user where USER-ID='$user_id'")or die("query is incorrect...");
}
...
?>
	...
	<th><a href="adduser.php" class="btn btn-success">Add New</a></th>
                    ...
                    <tbody>
                      <?php 
                        $result=mysqli_query($con,"select USER_NAME, EMAIL, PASSWORD,CONTECT from user")or die ("query 2 incorrect.......");

                        while(list($user_name,$email,$user_password,$contect)
                        mysqli_fetch_array($result))
                        {
                        echo "<tr><td>$user_name</td><td>$user_password</td><td>$emai</td>,<td>$contect</td>";
